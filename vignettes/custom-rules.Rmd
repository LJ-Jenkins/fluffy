...
These functions take the fluffy object, the name of the new schema type/coerce value, and a function that takes one argument, in the same vein as base R `is.*()` and `as.*()` functions. Note that the function must take a single argument, as the `Registry` will check the function signature and throw an error if it does not match. Functions that take multiple arguments can be used when passed directly to a `Schema` or `Validator`, but the additional arguments will be ignored.
